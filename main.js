...
let debugBool = false;
let celi = 0; //天体のindex
let iyGrid = [[],[]]; //grid of iti yuragi
let ryGrid = []; //grid of radius yuragi
let yGridWidth = 0.1; //deg
let canvRadi;
...
function draw(){
    ...
    cimg.style.opacity = 148 * Math.pow(fov, 2) / minRadimaxMagS.value ;
    ...
    if(bbu.value == "mag"){ //mag to fluxする必要
        con.fillStyle = `#ffffff${Math.floor(Math.pow(10, (backBriS.value - 25.29234)/-2.5)*255/minRadimaxMagS.value/Math.pow(vmagn/3600, 2)/10000).toString(16).padStart(2,'0')}`;
    }else{
        //minRadimaxMagS is fluxE-4
        con.fillStyle = `#ffffff${Math.floor(backBriS.value*255/minRadimaxMagS.value/Math.pow(vmagn/3600, 2)/10000).toString(16).padStart(2,'0')}`;
    }
    con.beginPath();
    ...
    for(let i=0; i<stars.data.length; i++){
        ...
        // gaia archive often includes NULL
        nullKazu = 0;
        for(let j=0; j<3; j++){
        ...
        */
        ...
        if(0 < oindex[0] && oindex[0] < ryGrid.length-1 && 0 < oindex[1] && oindex[1] < ryGrid.length-1){
            rand[0] += ryGrid[Math.floor(oindex[0])][Math.floor(oindex[1])] * (1 - oindex[0]%1)*(1 - oindex[1]%1);
            rand[0] += ryGrid[ Math.ceil(oindex[0])][Math.floor(oindex[1])] *   oindex[0]%1    *(1 - oindex[1]%1);
        ...
        for(let k=0; k<2; k++){
            if(0 < oindex[0] && oindex[0] < iyGrid[k].length-1 && 0 < oindex[1] && oindex[1] < iyGrid[k].length-1){
                rand[k] += iyGrid[k][Math.floor(oindex[0])][Math.floor(oindex[1])] * (1 - oindex[0]%1)*(1 - oindex[1]%1);
                rand[k] += iyGrid[k][ Math.ceil(oindex[0])][Math.floor(oindex[1])] *   oindex[0]%1    *(1 - oindex[1]%1);
                rand[k] += iyGrid[k][Math.floor(oindex[0])][ Math.ceil(oindex[1])] * (1 - oindex[0]%1)*    oindex[1]%1  ;
                rand[k] += iyGrid[k][ Math.ceil(oindex[0])][ Math.ceil(oindex[1])] *   oindex[0]%1    *    oindex[1]%1  ;
